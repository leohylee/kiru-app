...
export default function Game({ settings, onQuit }) {
  ...
  // Prevent scroll when virtual keyboard opens
  useEffect(() => {
    const handleResize = () => {
      window.scrollTo(0, 0);
    };
  ...
  return (
    <div className="h-[100dvh] bg-[#2a2e30] text-white flex flex-col overflow-hidden">
      <div className="flex justify-between items-center p-3 md:p-4 border-b border-gray-800 shrink-0">
        ...
        <button
          onClick={handleQuit}
          className="px-3 py-2 md:px-4 text-gray-400 hover:text-white active:bg-gray-800 hover:bg-gray-800 rounded-lg transition-colors text-sm"
        >
          End Session
        </button>
      </div>

      <div className="flex-1 flex flex-col items-center justify-start pt-4 md:justify-center md:pt-0 p-2 md:p-6 min-h-0">
        <div
          className={`text-[100px] md:text-[120px] font-sans mb-1 md:mb-4 transition-all duration-200 ${
            feedback === 'correct'
        ...
        {showSkipAnswer && (
          <div className="text-amber-400 text-2xl mb-1 md:mb-4 animate-pulse">
            {currentCard?.answers[0]}
          </div>
        )}

        {!showSkipAnswer && <div className="h-8 md:h-10 mb-1 md:mb-4" />}
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-xs">
          ...
          <button
            ...
            className="w-full mt-2 md:mt-4 py-3 md:py-2 text-gray-400 bg-gray-800 active:bg-gray-700 hover:text-amber-400 hover:bg-gray-700 rounded-lg transition-colors text-base md:text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
